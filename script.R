...
saveWorkbook(wb, "terminales.xlsx", overwrite = TRUE)

n <- 1
wb = createWorkbook()
for(n in 1:length(pais2)){
  sh = addWorksheet(wb, pais2[n])
  xl_write(dividida2[n], wb, pais2[n])
  xl_write(dividida2[n], wb, pais2[n])
  addStyle(wb, pais2[n], negrita, 1, 3:18, gridExpand = FALSE, stack = FALSE)
  
  n = n+1
}
saveWorkbook(wb, "terminales2.xlsx", overwrite = TRUE)
